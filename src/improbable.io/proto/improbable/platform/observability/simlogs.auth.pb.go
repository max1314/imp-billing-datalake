...
func init() {
	improbable_io_lib_perms_registry.RegisterPermissions("/improbable.platform.observability.SimlogsService/GetSimlogs",
		[]improbable_io_proto_improbable_ext_plugin_auth.Permission{},
	)
	improbable_io_lib_perms_registry.RegisterPermissions("/improbable.platform.observability.SimlogsService/Stream",
		[]improbable_io_proto_improbable_ext_plugin_auth.Permission{},
	)
}

// The arguments for this v1 permission check.
type Permits_V1_DeploymentLogsRead_Args struct {
	Deployment string
	Project    string
}

// Performs the delegated permission check described by delegation "DeploymentLogsRead".
func Permits_V1_DeploymentLogsRead(ctx context.Context, args Permits_V1_DeploymentLogsRead_Args) error {
	perm := fmt.Sprintf("[r]:prj/%v/dpl/%v/log", args.Project, args.Deployment)
...
// The arguments for this v1 permission check.
type Permits_V1_DeploymentLogsStream_Args struct {
	Deployment string
	Project    string
}

// Performs the delegated permission check described by delegation "DeploymentLogsStream".
func Permits_V1_DeploymentLogsStream(ctx context.Context, args Permits_V1_DeploymentLogsStream_Args) error {
	perm := fmt.Sprintf("[r]:prj/%v/dpl/%v/log", args.Project, args.Deployment)
	grant, err := improbable_io_lib_oauth2_perms.ParseGrant(perm)
	if err != nil {
		return err
